...
data "template_file" "kube_config" {
  template = "${file("${path.module}/tpl/kube_config.yaml.tpl")}"
...
data "template_file" "kube_config_secret" {
  template = "${file("${path.module}/tpl/kube_config_secret.yaml.tpl")}"
...
data "template_file" "map_roles" {
  ...
  template = "${file("${path.module}/tpl/aws_auth-map_roles.yaml.tpl")}"
...
data "template_file" "map_users" {
  ...
  template = "${file("${path.module}/tpl/aws_auth-map_users.yaml.tpl")}"
...
data "template_file" "aws_auth" {
  template = "${file("${path.module}/tpl/aws_auth.yaml.tpl")}"
